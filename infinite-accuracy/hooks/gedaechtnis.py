...
def register_bauen(ordner, schreiben=False):
    ...
    nach_typ = {}
    for e in eintraege:
        nach_typ.setdefault(e["typ"], []).append(e)
    rubriken = t["rubriken"]
    ...
    for typ in reihenfolge:
        gruppe = nach_typ.get(typ)
        if not gruppe:
            continue
        z.append("## %s" % rubriken.get(typ, typ))
        z.append("")
        for e in sorted(gruppe, key=lambda x: x["slug"]):
            z.append("- [%s](%s) — %s"
...
def main():
    ...
    if "--register" in sys.argv:
        schlank, ausf, eintraege = register_bauen(ordner, schreiben=True)
